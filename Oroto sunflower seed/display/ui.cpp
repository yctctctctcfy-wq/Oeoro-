...
void displayModuleStatus() {
    ...
    std::vector<std::pair<std::string, std::string>> modules = {
        {"Kernel Core", "ACTIVE"},
        {"Command Parser", "LOADED"},
        {"System Calls", "READY"},
        {"Device Interface", "INITIALIZED"},
        {"Network Scanner", "STANDBY"},
        {"Hash Cracker", "READY"},
        {"Ping Engine", "ACTIVE"},
        {"Security Suite", "LOADED"},
        {"Hardware HAL", "CONNECTED"},
        {"Memory Manager", "OPTIMIZED"}
    };
    
    for (const auto& module : modules) {
        std::cout << WHITE << "  " << std::setw(20) << std::left << module.first << " : ";
        if (module.second == "ACTIVE") {
            std::cout << BOLD << GREEN << module.second << RESET << "\n";
        } else if (module.second == "READY" || module.second == "LOADED") {
            std::cout << GREEN << module.second << RESET << "\n";
        } else {
            std::cout << YELLOW << module.second << RESET << "\n";
        }
    }
    std::cout << "\n";
}
...
void clearScreenAdvanced() {
    ...
    // Display status bar after clearing
...
void displayErrorMessage(const std::string& error, const std::string& hint = "") {
    ...
    if (!hint.empty()) {
        std::cout << YELLOW << "Hint: " << hint << RESET << "\n";
    }
    std::cout << "\n";
}
